...
// ForExprAST - Expression class or for loops
...
/// The Prototype now captures whether it is an opeator or not, and its precedence
class PrototypeAST {
  std::string Name;
...
static std::unique_ptr<ExprAST> ParseForExpr() {
	...
	if (CurTok != '=')
		return LogError("expetected '=' after for");
	getNextToken(); // eat '='
	...
	// The step value is optional (Could optimize this to repeat the loop 'step' many times instead of checking counter)
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
	...
	case tok_binary:
		...
		FnName = "binary"; // identifies the operator as binary
		FnName += char(CurTok); // the symbol for the operator, for example "binary@" is the binary operator '@'
		Kind = 2;
		getNextToken();
...
Value *BinaryExprAST::codegen() {
  ...
  // call to it. This is just a lookup for hte binary operator in the symbol table
  Function *F = getFunction(std::string("binary") + Op);
  assert(F && "binary operator not found");
  
  // This generates the function call to the appropriate operator
  Value *Ops[] = {L, R};
...
Value *VarExprAST::codegen() {
	...
	for (unsigned i = 0, e = VarNames.size(); i != e; ++i) {
		...
		// Emit the initializer befgore adding the variable to scope, this prevents 
		// the intializer from referencing the variable itself, and permits stuff like this 
		//  var a = 1 in
	...
	// Codegen the body, now that all the vars are in scope 
	Value *BodyVal = Body->codegen();
	if (!BodyVal)
		return nullptr;

	// Pop all fo our variables from scope
	for (unsigned i = 0, e = VarNames.size(); i != e; ++i)
...
Function *FunctionAST::codegen() {
  ...
  // before codegening the function, if it is a user defined operator
  // we register it in the precedence table 
  if (P.isBinaryOp())
